 ...
 */
export async function renderGridView(container, articles, filters) {
  // Clear container
  container.innerHTML = '';
  
  // Set up filter change handler
  filters.setOnChange(async (filterState) => {
    await renderGridView(container, articles, filters);
  });
  ...
  for (const article of filteredArticles) {
    const articleElement = await createArticleElement(article, 'grid');
